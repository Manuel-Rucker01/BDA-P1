"""
BDA Exact Horizons Performance Verification.
This script runs rolling out-of-sample backtests for four horizons:
- 3 Months (from Feb 20, 2026 to May 22, 2026)
- 6 Months (from Nov 21, 2025 to May 22, 2026)
- 12 Months (from May 23, 2025 to May 22, 2026)
- 24 Months (from May 24, 2024 to May 22, 2026)

For each horizon, it starts with an initial capital of $10,000 and calculates:
1. Buy & Hold Basket Return & Ending Value
2. Standard Long-Only (P >= 0.5) Return & Ending Value
3. High-Confidence Longs (P >= 0.53) Return & Ending Value
"""
...
def run_backtest_for_horizon(df_all_feat, df_full, friday_dates, company_embeddings, scaler, pca, trained_models, mix_models, tabular_cols, pca_cols, start_date, initial_equity=10000.0):
    # Filter Friday dates from start_date
    horizon_fridays = [d for d in friday_dates if d >= start_date]
    if not horizon_fridays:
        return 0.0, 0.0, 0.0
    
    portfolio_bh = [initial_equity]
    portfolio_long = [initial_equity]
    ...
    return portfolio_bh[-1], portfolio_long[-1], portfolio_high_long[-1]

def main():
    print("=" * 80)
    print("BDA EXACT HORIZONS PERFORMANCE CALCULATOR")
    print("=" * 80)
    ...
    print(f"Loaded ensemble. Tickers: {basket}")

    # Fetch data since 2024-03-01 to have technical indicators history for 24-month horizon (starts 2024-05-24)
    print("\n[Ingestion] Fetching live pricing bars (2024-03-01 to 2026-05-22)...")
    df_list = []
    ...
    print(f"Found {len(friday_dates)} Friday rebalancing periods.")
    ...
    print("\n" + "=" * 95)
    print(f"{'Horizon':<25} | {'Strategy Name':<28} | {'Cumulative Ret %':<18} | {'Ending Value ($)':<18}")
    print("=" * 95)

    for label, start_date in horizons.items():
        bh_val, long_val, high_long_val = run_backtest_for_horizon(
            df_all_feat, df_full, friday_dates, company_embeddings,
        ...
        long_ret = (long_val - initial_capital) / initial_capital * 100.0
        high_long_ret = (high_long_val - initial_capital) / initial_capital * 100.0
        
        print(f"{label:<25} | {'Buy & Hold Benchmark':<28} | {bh_ret:>16.2f}% | ${bh_val:>16,.2f}")
        print(f"{'':<25} | {'Standard Longs (P >= 0.5)':<28} | {long_ret:>16.2f}% | ${long_val:>16,.2f}")
        print(f"{'':<25} | {'High-Confidence Longs':<28} | {high_long_ret:>16.2f}% | ${high_long_val:>16,.2f}")
        print("-" * 95)

"""
BDA S&P 500 Broad-Market Trend Regime Filter Backtester (50 Popular Stocks Basket).
This script compares:
1. Buy & Hold Benchmark
2. Unfiltered Probabilistic Long/Short
3. Regime-Filtered Long/Short (S&P 500 Close > 50-day SMA shuts off shorts)
4. High-Confidence Longs (P >= 0.53) reference
Across 3, 6, 12, and 24-month horizons ending on May 22, 2026, using 50 of the most popular stock market leaders.
"""
...
from trading_agent.bot import build_regime_filtered_weights
...
def calculate_weights_probabilistic(df_predictions, is_bull=False, target_exposure=1.0, confidence_threshold=0.02):
    weights, _ = build_regime_filtered_weights(
        df_predictions,
        is_bull=is_bull,
...
def main():
    print("=" * 110)
    print("BDA 50 POPULAR STOCKS BACKTESTER & REGIME FILTER")
    print("=" * 110)

    # 1. Load S&P 500 history
    ...
    print(f"Loaded {len(gspc_dict)} S&P 500 trend dates. Latest close: ${gspc_trends.iloc[-1]['Close_GSPC']:,.2f}")
    ...
    # Defined 50 popular symbols basket
    basket = ['AAPL', 'MSFT', 'AMZN', 'GOOG', 'GOOGL', 'NVDA', 'TSLA', 'NFLX', 'AMD', 'INTC', 
              'CSCO', 'CMCSA', 'PEP', 'COST', 'AVGO', 'QCOM', 'TXN', 'ADBE', 'AMAT', 'MU', 
              'SBUX', 'ISRG', 'LRCX', 'AMGN', 'GILD', 'MDLZ', 'INTU', 'BKNG', 'ADP', 'CSX', 
              'ADI', 'REGN', 'PYPL', 'FISV', 'VRTX', 'JD', 'ASML', 'MELI', 'LULU', 'WDAY', 
              'NXPI', 'MNST', 'BIIB', 'ALGN', 'EBAY', 'FTNT', 'MRVL', 'ADSK', 'CDNS', 'CHKP']
              
    print(f"Selecting custom basket of 50 active popular symbols for testing:")
    print(basket)

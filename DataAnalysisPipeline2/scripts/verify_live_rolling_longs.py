"""
BDA Rolling Out-of-Sample Verification for Long Predictions.
This script tests our trained ensemble model's real-world long-only performance by:
1. Ingesting yfinance pricing data from 2025-12-01 to 2026-05-22.
2. For each Friday in our testing window (2026-02-13 to 2026-05-15):
   ...
   c. Calculating actual returns over the following 5 trading days (the subsequent week).
3. Printing weekly returns and final metrics.
4. Saving a comparison performance chart.
"""
...
OUTPUT_PLOT_PATH = os.path.join(EXPLOITATION_DIR, "rolling_longs_performance.png")
...
def main():
    print("=" * 80)
    print("BDA ROLLING OUT-OF-SAMPLE LONG PERFORMANCE TESTER")
    print("=" * 80)
    ...
    print("\n[Ingestion] Fetching live pricing bars from 2025-12-01 to 2026-05-22...")
    df_list = []
    for ticker in basket:
        ticker_df = yf.download(ticker, start="2025-12-01", end="2026-05-23", progress=False)
    ...
    # 3. Resolve testing dates (Fridays)
    # Find all Friday dates that are available in our dataset between 2026-02-13 and 2026-05-15
    ...
    fridays_df = dates_df[
        ...
        (dates_df["Date"] >= "2026-02-13") & 
        (dates_df["Date"] <= "2026-05-15")
    ].sort_values("Date")
    
    friday_dates = fridays_df["Date"].tolist()
    print(f"Found {len(friday_dates)} Friday rebalancing periods for evaluation.")

    # 4. Load metadata & macro features once
    ...
    for idx, friday in enumerate(friday_dates):
        ...
        if friday_obs.empty:
            results_rows.append({
                "Date": friday, "B&H Return": 0.0, "Long Return": 0.0, "High Long Return": 0.0,
                "Num Longs": 0, "Num High Longs": 0
            })
        ...
        # Determine actual returns over the next 5 trading days (next Friday)
        # Fetch prices on next Friday
        if idx + 1 < len(friday_dates):
            next_friday = friday_dates[idx + 1]
        else:
            next_friday = "2026-05-22"  # Today is the endpoint for the final week
        ...
        # 1. B&H Return (equal weighted between all available basket tickers)
        ...
        # Fallback to top 2 if empty to ensure we are always invested
        ...
        results_rows.append({
            ...
            "Num Longs": len(long_tickers),
            "Num High Longs": len(high_long_tickers)
        })
    ...
    # 6. Output Table
    print("\n" + "=" * 95)
    print(f"{'Rebalance Date':<15} | {'B&H Ret (%)':<13} | {'Long-Only Ret (%)':<18} | {'High-Conf Ret (%)':<18} | {'# Longs':<8} | {'# High-Conf':<10}")
    print("-" * 95)
    for _, row in df_results.iterrows():
        print(f"{row['Date']:<15} | {row['B&H Return']:>10.2f}% | {row['Long Return']:>15.2f}% | {row['High Long Return']:>15.2f}% | {int(row['Num Longs']):>7} | {int(row['Num High Longs']):>10}")
    print("-" * 95)
    ...
    # Win rates (number of positive return weeks)
    ...
    print(f"{'CUMULATIVE RETURN':<15} | {final_bh:>10.2f}% | {final_long:>15.2f}% | {final_high_long:>15.2f}% |")
    print(f"{'WEEKLY WIN RATE':<15} | {win_bh:>10.2f}% | {win_long:>15.2f}% | {win_high_long:>15.2f}% |")
    print("=" * 95)
    ...
    try:
        ...
        plt.plot(plot_dates, portfolio_bh, label=f"Buy & Hold Basket (Return: {final_bh:.1f}%)", color="#7f8c8d", linewidth=2.5, linestyle="--")
        plt.plot(plot_dates, portfolio_long, label=f"Standard Long-Only (P >= 0.5) (Return: {final_long:.1f}%)", color="#2ecc71", linewidth=3)
        plt.plot(plot_dates, portfolio_high_long, label=f"High-Confidence Longs (P >= 0.53) (Return: {final_high_long:.1f}%)", color="#1abc9c", linewidth=3)
        
        plt.title("Rolling Out-of-Sample Long Strategy Real-World Performance (Feb - May 2026)", fontsize=13, fontweight="bold", pad=15)

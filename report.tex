...
Rebalance cadence & Monthly (first Friday) & The target is a 30-day forward rank, so the trading cadence must match that horizon. A head-to-head test (\S\ref{sec:cadence}) found weekly rebalancing churns the signal on sub-horizon noise---net $+3\%$ vs monthly's $+22\%$ over the 20-month window, with $4\times$ worse Sharpe and double the drawdown. We rejected weekly. \\
...
\subsection{Rebalance Cadence: Why We Rebalance Monthly}
\label{sec:cadence}
The supervised target is a \emph{30-day} forward cross-sectional rank, so the natural trading cadence is one that lets each ranking play out over roughly that horizon. We tested this directly on the deployed (news-trained) full-universe top-$K{=}10$ book, comparing a \textbf{weekly} schedule (every Friday---the model's native grid, used for the tables above) against a \textbf{monthly} schedule (first Friday of each month, held to the next), both net of the same 10 bps one-way turnover charge (Table~\ref{tab:cadence}).

\begin{table}[htbp]
\caption{Weekly vs.\ monthly rebalancing of the deployed top-$K{=}10$ book, net of 10 bps one-way turnover cost. Monthly holds each book to roughly the model's 30-day target horizon. Sharpe/IR are annualised at $52$/yr (weekly) and $12$/yr (monthly). $^{\ddagger}$Pre-training window carries the survivorship / static-embedding caveats of Table~\ref{tab:bt_horizons}.}
\label{tab:cadence}
\centering
\small
\begin{tabular}{llrrrr}
\toprule
\textbf{OOS Window} & \textbf{Cadence} & \textbf{Net Ret.} & \textbf{Sharpe} & \textbf{Max DD} & \textbf{IR} \\
\midrule
Pre-Training$^{\ddagger}$  & Weekly (41) & $+3.12\%$           & $0.29$          & $-19.3\%$         & $-1.14$ \\
(20 mo)                    & Monthly (10)& $\mathbf{+21.91\%}$ & $\mathbf{1.21}$ & $\mathbf{-7.2\%}$ & $\mathbf{+0.13}$ \\
\midrule
Post-Training (clean)      & Weekly (8)  & $+56.71\%$          & $6.47$          & $-3.5\%$          & $4.42$ \\
(2 mo)                     & Monthly (3) & $+56.62\%$          & $\mathbf{9.45}$ & $\mathbf{0.0\%}$  & $\mathbf{9.05}$ \\
\bottomrule
\end{tabular}
\end{table}

Two findings stand out. On the statistically-meaningful 20-month window, \textbf{monthly beats weekly by $+18.8$ points of net return} ($+21.91\%$ vs $+3.12\%$), with roughly $4\times$ the Sharpe ($1.21$ vs $0.29$), half the maximum drawdown ($-7.2\%$ vs $-19.3\%$), and a positive rather than deeply negative Information Ratio. On the short clean window the two cadences are a return tie ($\sim\!56.6\%$ each), but monthly is markedly more risk-efficient (Sharpe $9.45$ vs $6.47$, zero drawdown, double the IR).

Crucially, this is \emph{not} primarily a transaction-cost story: the \emph{gross} returns already diverge ($+22.70\%$ vs $+4.66\%$ over the 20-month window), and friction explains only $\sim\!1.5$ points of the weekly shortfall. The dominant mechanism is signal-horizon alignment---re-ranking weekly churns a 30-day-horizon signal on week-to-week noise, exiting names before the thesis matures and chasing freshly top-ranked names that then mean-revert, which is what drives the $-19\%$ weekly drawdown. (Per-rebalance turnover is in fact \emph{higher} monthly, $0.66$ vs $0.36$, because a month accumulates more drift; but trading $4\times$ less often more than compensates.) We therefore deploy the agent on a \textbf{monthly} rebalance; the weekly tables above are the conservative, higher-friction reference. A weekly risk-only check of the HMM regime gate and the drawdown circuit breaker between monthly rebalances remains sensible---but the alpha decision is taken monthly.

...
\subsection{Live News-Sentiment Tilt (Semantic Extension, Live-Only)}
...
\textbf{This feature is deliberately live-only and is never used in the walk-forward bake-off, the backtests, or the structural embeddings.} A leak-free historical evaluation would require a point-in-time news archive --- each article timestamped at its true public-release moment and scored with only-then-available information --- which current-news APIs do not provide. Backtesting against a present-day news feed would silently inject look-ahead bias and invalidate the very out-of-sample guarantees this paper is built on. We therefore treat news sentiment as a documented live overlay and a future-work item (a PIT archive, or news events promoted into a temporally-versioned graph, would enable a rigorous backtested study). The backtested results reported above are computed with this tilt disabled and are provably unaffected by it.
